...
print("unique gene: ", len(gene_names))
print("unique miRNA: ", len(miRNA_names))
print("both gene and miRNA associated disease: ", len(set(disease_names_of_gene_unique).intersection(set(disease_names_of_miRNA_unique))))
print("gene-associated disease: ", len(disease_names_of_gene_unique))
print("miRNA-associated disease: ", len(disease_names_of_miRNA_unique))

# 求交集，使得disease在两个文件中都有数据存在
disease_name_set = [disease for disease in disease_names_of_gene_unique if disease in disease_names_of_miRNA_unique]
...
#######################################################################################################
#######################################################################################################
# 重新载入数据：只保留与disease_name相关的miRNA和gene对应的行
miRNA_names = []
disease_names_of_miRNA = []
with open(MIRNA_DISEASE_FILE) as tsv:
    disease_temp = ""
